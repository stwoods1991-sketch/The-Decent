...
const combatDamageMultiplier=(h:Hero,e:Enemy)=>1+(setCount(h,'Waiver Wire')>=2?.12:0)+(setCount(h,'Injury Report')>=2&&h.hp/h.maxHp<.5?.25:0)+(cursedCount(h)>=3?.25:0)+(e.hp/e.maxHp<.3?effectTotal(h,'execute'):0);
...
export function tick(s:RunState):RunState { if(s.screen!=='battle'||!s.hero||!s.enemy||s.paused)return s; let n=structuredClone(s), dt=BALANCE.tickMs/1000; const h=n.hero!,e=n.enemy!;n.telemetry.elapsed+=dt;n.telemetry.encounterElapsed+=dt;n.heroClock+=dt*h.speed*(setCount(h,'Advanced Metrics')>=2?1.15:1);n.enemyClock+=dt*e.speed;n.abilityClock+=dt;
 if(n.heroClock>=1){n.heroClock-=1;const atk=h.attack+(h.classId==='gremlin'?Math.floor(h.gold/28):0);const crit=rng(n.seed+n.telemetry.elapsed*10)<h.crit;const d=damage(atk*(crit?1.7:1)*combatDamageMultiplier(h,e),e.defense);e.hp-=d;let burn=0;if(crit&&effectTotal(h,'critBurn')>0){burn=Math.max(1,Math.round(atk*effectTotal(h,'critBurn')));e.hp-=burn}if(h.classId==='necro')grantHealing(n,Math.round(d*.10));n.log.unshift(`${crit?'CRIT — ':''}${h.name} hits ${e.name} for ${d}${burn?` + ${burn} Burn`:''}.`)}
 if(n.abilityClock>=8){n.abilityClock=0;const atk=h.attack+(h.classId==='gremlin'?Math.floor(h.gold/28):0);let mult=2.2,hits=1;if(h.classId==='wizard')mult=3.4;if(h.classId==='rat'){mult=1.35;hits=2}if(h.classId==='gremlin')mult=2.4;const abilityBonus=1+effectTotal(h,'abilityPower')+(setCount(h,'Commissioner')>=2?.25:0);let total=0;for(let k=0;k<hits;k++){const d=damage(atk*mult*h.abilityPower*abilityBonus*combatDamageMultiplier(h,e),e.defense);e.hp-=d;total+=d}if(h.classId==='goblin')h.shield+=Math.round(h.maxHp*.07);if(h.classId==='commissioner'){n.enemyClock=Math.max(0,n.enemyClock-.6);h.shield+=Math.round(h.maxHp*.08)}if(h.classId==='wizard')grantHealing(n,Math.round(total*.10));if(h.classId==='necro')grantHealing(n,Math.round(total*.20));if(h.classId==='rat')h.attack+=1;n.log.unshift(`${CLASSES[h.classId].ability.split(':')[0]} triggers for ${total}!`)}
 if(e.hp<=0)return victory(n);
 if(n.enemyClock>=1){n.enemyClock-=1;let mult=1;if(e.boss&&n.telemetry.encounterElapsed>45)mult+=Math.min(.8,(n.telemetry.encounterElapsed-45)/90);let d=damage(e.attack*mult,h.defense);if(h.shield){const blocked=Math.min(h.shield,d);h.shield-=blocked;d-=blocked}h.hp-=d;const reflect=Math.round(d*(effectTotal(h,'thorns')+(setCount(h,'Goalie Gear')>=2?.15:0)));if(reflect>0)e.hp-=reflect;n.telemetry.damage.push(d);n.telemetry.damage=n.telemetry.damage.slice(-20);n.log.unshift(`${e.name} deals ${d}${reflect?`; ${reflect} reflected`:''}.`)}
 if(e.hp<=0)return victory(n);
...
function gearReward(item:Item):Reward{
 ...
 return {id:'gear',title:item.name,detail:`${item.rarity} ${item.slot}: ${val}${item.effect?` · ${item.effect.description}`:''}${item.curse?` · ${item.curse}`:''}`,kind:'gear',amount:0,item};
}
