import { BALANCE, BOSSES, CLASSES, ENEMY_NAMES, ITEMS } from './data';
import type { ClassId, Enemy, Hero, Item, Reward, RunState } from './types';
...
export function tick(s:RunState):RunState { if(s.screen!=='battle'||!s.hero||!s.enemy||s.paused)return s; let n=structuredClone(s), dt=BALANCE.tickMs/1000; const h=n.hero!,e=n.enemy!;n.telemetry.elapsed+=dt;n.telemetry.encounterElapsed+=dt;n.heroClock+=dt*h.speed;n.enemyClock+=dt*e.speed;n.abilityClock+=dt;
 if(n.heroClock>=1){n.heroClock-=1;const atk=h.attack+(h.classId==='gremlin'?Math.floor(h.gold/28):0);const crit=rng(n.seed+n.telemetry.elapsed*10)<h.crit;const d=damage(atk*(crit?1.7:1),e.defense);e.hp-=d;if(h.classId==='necro'){const heal=Math.min(Math.round(d*.10),h.maxHp-h.hp);if(heal>0){h.hp+=heal;n.telemetry.healing.push(heal)}}n.log.unshift(`${crit?'CRIT — ':''}${h.name} hits ${e.name} for ${d}.`)}
 if(n.abilityClock>=8){n.abilityClock=0;const atk=h.attack+(h.classId==='gremlin'?Math.floor(h.gold/28):0);let mult=2.2,hits=1;if(h.classId==='wizard')mult=3.4;if(h.classId==='rat'){mult=1.35;hits=2}if(h.classId==='gremlin')mult=2.4;let total=0;for(let k=0;k<hits;k++){const d=damage(atk*mult*h.abilityPower,e.defense);e.hp-=d;total+=d}if(h.classId==='goblin')h.shield+=Math.round(h.maxHp*.07);if(h.classId==='commissioner'){n.enemyClock=Math.max(0,n.enemyClock-.6);h.shield+=Math.round(h.maxHp*.08)}if(h.classId==='wizard'){const heal=Math.min(Math.round(total*.10),h.maxHp-h.hp);if(heal>0){h.hp+=heal;n.telemetry.healing.push(heal)}}if(h.classId==='necro'){const heal=Math.min(Math.round(total*.20),h.maxHp-h.hp);if(heal>0){h.hp+=heal;n.telemetry.healing.push(heal)}}if(h.classId==='rat')h.attack+=1;n.log.unshift(`${CLASSES[h.classId].ability.split(':')[0]} triggers for ${total}!`)}
 if(e.hp<=0)return victory(n);
 if(n.enemyClock>=1){n.enemyClock-=1;let mult=1;if(e.boss&&n.telemetry.encounterElapsed>45)mult+=Math.min(.8,(n.telemetry.encounterElapsed-45)/90);let d=damage(e.attack*mult,h.defense);if(h.shield){const blocked=Math.min(h.shield,d);h.shield-=blocked;d-=blocked}h.hp-=d;n.telemetry.damage.push(d);n.telemetry.damage=n.telemetry.damage.slice(-20);n.log.unshift(`${e.name} deals ${d}${h.shield?' through your shield':''}.`)}
...
function victory(n:RunState):RunState { const h=n.hero!;const e=n.enemy!;award(n,'first-blood');if(e.boss&&h.hp/h.maxHp<0.10)award(n,'close-call');const gi=BALANCE.goldIncome+n.floor*2;h.gold+=h.classId==='gremlin'?Math.round(gi*2.0):gi;h.essence+=1; n.log.unshift(`${n.enemy!.name} falls. The projections apologize.`); const last=n.floor===18&&n.encounter===BALANCE.encountersPerFloor;if(last){award(n,'clear');n.screen='summary';n.won=true;return n} const checkpoint=n.encounter%BALANCE.rewardEvery===0||n.enemy!.boss; if(checkpoint){n.screen='reward';n.rewards=makeRewards(n)} else advance(n); return n; }
export function advance(n:RunState){if(n.encounter>=BALANCE.encountersPerFloor){n.floor++;n.encounter=1}else n.encounter++;if(n.floor===5)award(n,'floor-5');if(n.floor===10)award(n,'floor-10');if(n.floor===15)award(n,'floor-15');const H=n.hero!;const rest=Math.round(H.maxHp*BALANCE.encounterRest);if(rest>0&&H.hp<H.maxHp){const r=Math.min(rest,H.maxHp-H.hp);H.hp+=r;n.telemetry.healing.push(r)}n.enemy=enemyFor(n.floor,n.encounter,H);H.shield=H.classId==='goblin'?Math.round(H.maxHp*.10):0;n.heroClock=n.enemyClock=n.abilityClock=0;n.telemetry.encounterElapsed=0;n.screen='battle';n.paused=false;}
...
function rollItem(rand:()=>number,floor:number,exclude:Set<string>):Item{
 for(let tries=0;tries<10;tries++){const rar=rollRarity(rand,floor);const pool=ITEMS.filter(i=>i.rarity===rar&&!exclude.has(i.id));if(pool.length)return pool[Math.floor(rand()*pool.length)]}
...
function gearReward(item:Item):Reward{
 ...
 return {id:'gear',title:item.name,detail:`${item.rarity} ${item.slot}: ${val}${item.curse?` · ${item.curse}`:''}`,kind:'gear',amount:0,item};
}
export function makeRewards(s:RunState,salt=0):Reward[]{
 ...
 const out:Reward[]=[];const exclude=new Set<string>();
 const gearCount=rand()<0.28?3:2; // occasional loot-heavy checkpoint
 for(let k=0;k<gearCount;k++){const item=rollItem(rand,s.floor,exclude);exclude.add(item.id);out.push(gearReward(item))}
...
export function shopOffer(s:RunState):Item{const rand=mulberry32(((s.seed>>>0)^Math.imul(s.floor,40503)^Math.imul(s.encounter,90001)^0x5f3759df)>>>0);return rollItem(rand,s.floor,new Set());}
...
export function buyShop(s:RunState,kind:'heal'|'reroll'|'item'):RunState{const n=structuredClone(s),h=n.hero!;const cost=shopCosts(n.floor)[kind];if(h.gold<cost)return s;
 if(kind==='heal'){if(h.hp>=h.maxHp)return s;h.gold-=cost;const amt=Math.min(h.maxHp-h.hp,Math.round(h.maxHp*.28));h.hp+=amt;n.telemetry.healing.push(amt);n.log.unshift(`Bench medic restores ${amt} HP.`)}
